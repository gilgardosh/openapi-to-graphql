...
function getOas3() {
  const oasStr = readFileSync(join(__dirname, './fixtures/example_oas3.json'), 'utf8');
  const oas = JSON.parse(oasStr);
  // update PORT for this test case:
  oas.servers[0].variables.port.default = String(PORT2);
  return oas;
};
...
beforeAll(() => {
  return Promise.all([
    openAPIToGraphQL
      .createGraphQLSchema([getOas(), getOas3()])
      .then(({ schema, report }) => {
...
// NOTE: New translator supports only one OAS at a time
...
//     .createGraphQLSchema([getOas(), getOas3()], options)
//     .then(({ schema }) => {
